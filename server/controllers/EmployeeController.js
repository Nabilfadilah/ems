...
const updateEmployee = async (req, res) => {
    try {
        ...
        if(!updateEmployee || !updateUser) {
            return res 
            .status(404)
            .json({success: false, error: "ducument not found"})
        }

        return res.status(200).json({success: true, message: "emplooyee update"})
...
// fecth Employees By DepId
const fecthEmployeesByDepId = async (req, res) => {
    const {id} = req.params;
...
export {addEmployee, upload, getEmployees, getEmployee, updateEmployee, fecthEmployeesByDepId}
